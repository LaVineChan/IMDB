        ...
        for batch in train_iter:
            ...
            if steps % args.test_interval == 0:
                ...
                if dev_corrects > best_corrects:
                    ...
                    last_step = steps
                    logger.info('Saving best model at epoch: {} step: {}, corrects: {:.4f}'.format(epoch, steps,  best_corrects))
                    save(model, args.save_dir,  'best_trainRNN_', logger)
                model.train()
...
def predict(test_iter, model, args, logger):
    ...
    submission.to_csv("./submit/textRNN_submission_0.001.csv", index=False, header=False)



def save(model, save_dir, save_prefix, logger):
