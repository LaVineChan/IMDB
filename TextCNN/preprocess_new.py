...
# review_text = data.Field(sequential=True, lower=True, tokenize=tokenizer, fix_length=500)
# is_spoiler = data.LabelField(dtype=torch.int)
# test_file = '../data/test.json'
# test_df = get_test(test_file)
# test_examples, test_fields = get_dataset(test_df, review_text, None, True)
# test = data.Dataset(test_examples, test_fields)
# review_text.build_vocab(test)
# test_iter = data.Iterator(test, batch_size=128, sort=False, sort_within_batch=False,
#                           repeat=False)
